        ...
        <div className="dlg-bd" style={{padding:"12px"}}>
          <div className="grid" style={{display:"grid", gap:6, gridTemplateColumns:"1fr"}}>
            {WALLETS.map((w) => (
              ...
              >
                <img src={w.icon} alt={w.label} style={{width:28,height:28,borderRadius:6,objectFit:"cover"}} />
                <div>
                  <div>{w.label}</div>
                  <div className="hint" style={{fontSize:13,color:"#a4b3d2",marginTop:4}}>Extension / App mobile</div>
                </div>
              </div>
            ))}
          </div>
          <div className="hint" style={{fontSize:13,color:"#a4b3d2",marginTop:8}}>
            Astuce mobile : ouvrez cette page dans le navigateur intégré de votre wallet.
          </div>
        </div>
      </dialog>
