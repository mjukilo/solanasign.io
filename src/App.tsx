...
export default function App() {
  ...
  const connectOrSwitch = async () => {
    if (connected) {
      // Ouvre le sélecteur pour changer de wallet
      openWalletModal();
    } else {
      openWalletModal();
    }
          ...
          <div className="flex items-center gap-3 w-full max-w-[60%] justify-end">
            ...
            {connected ? (
              <button
                ...
                className="hidden md:inline-flex rounded-xl px-3 py-2 text-xs border border-slate-700/60 hover:bg-slate-800/50"
                title="Disconnect"
              >
                Disconnect
              </button>
                ...
                <div className="mt-6">
                  ...
                  <details className="mt-4 group">
                    ...
                    <div className="mt-3 text-sm text-slate-300/80 space-y-2">
                      <p>
                        Use <code>tweetnacl</code> or <code>@solana/web3.js</code> to
                        verify the signature bytes over the exact same message, using
                        the wallet public key.
                      </p>
                      <p className="text-slate-400">
                        Make sure the verifier encodes the message as UTF-8 and decodes the
                        signature from base58.
                      </p>
                    </div>
                  </details>
                </div>
